...
# This is the way I was doing it but I couldnt finish it
# Asking the number of students and assignments 
numberOfStudents = int(input("How many students are in your class? "))
numberOfAssigments = int(input("How may assignments are given in this class? "))
...
for student in range(1,numberOfStudents + 1):
    studentScores = []
    for assignment in range(1,numberOfAssigments + 1):
        scorePerAssignment = int(input(f"Student {student}, Assignment #{assignment}: "))
        studentScores.append(scorePerAssignment)
        average += float(scorePerAssignment)
    studentList.append(studentScores)
    totalAverage = average / int(numberOfAssigments)
    studentScores.append(totalAverage)

# Calculating the scores and dysplaing them on the screen
print(studentList)


# This is as the teacher resolved it
...
for student in range(0, numStudents):
    ...
    for assignment in range(0,numAssignments):
        ...
        studentPoints += score
    print ("This student earnd: ", studentPoints/numAssignments)

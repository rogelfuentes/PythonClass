...
class Tracks(db.DBbase):
    ...
    def load_database_from_xml(self, fname):
        stuff = ET.parse(fname)
        all_items = stuff.findall("dict/dict/dict")
        print("Dict count:", len(all_items))

        for entry in all_items:
            if func.lookup(entry, "Track ID") is None: continue
            ...
            length = func.lookup(entry, "Length")
...
tracks = Tracks()
tracks.connect()
# tracks.reset_database()
tracks.load_database_from_xml("Library.xml")
